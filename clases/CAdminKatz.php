...
//                CAdminEventos::IngresarEventoInstrumentoFI($_nRegistro, 'katz');
  ...
  //      $a = '';
    ...
    //CONSULTA DAVID PARA OBTENER LOS QUE NO ESTAN EN KATZ NI EN SALUD_AM PERO SI ESTAN RESIDENTES
 ...
 public static function GetfechaKatz($fecha){
        
      $conn=CDBSingleton::GetConn();
        $query="SELECT fecha_real FROM ".Configuracion::$BDUMOVIL_VISTA.".bds_ultima_fecha_katz WHERE fecha_real<='".$fecha."'and fecha_real >=(SELECT max(fecha_real)from ".Configuracion::$BDUMOVIL_VISTA.".bds_ultima_fecha_katz )";
        $exito=$conn->query($query);
        if(CDBSingleton::RevisarExito($exito, $query))
        {
            $rs=$exito->fetchrow(DB_FETCHMODE_ASSOC);
            if($rs)
                return ($rs['fecha_real']);
        }
        return false;
    } 
